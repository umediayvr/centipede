...
def publish(content):
    """
    Publish Render to Shotgun and link to Shot.
    """
    ...
    seqData = tk.shotgun.find('Sequence', seqFilters)
    if len(seqData) != 1:
        sys.stderr.write(
            "[runRenderPublish] Cannot find unique sequence {} in project {}. Skip Publish.".format(
                content['seq'],
    ...
    shotFilters = [
        ['code', 'is', content['shot']],
        ['sg_sequence', 'is', {'type': 'Sequence', 'id': seqData[0].get('id')}]
    ]
    shotData = tk.shotgun.find('Shot', shotFilters)
    if len(shotData) != 1:
        sys.stderr.write(
            "[runRenderPublish] Cannot find unique shot {} in sequence {} in project {}. Skip Publish.".format(
                content['shot'],
                content['seq'],
                projectEntity.get('name')
            )
    ...
    ctx = tk.context_from_entity_dictionary(shotData[0])
